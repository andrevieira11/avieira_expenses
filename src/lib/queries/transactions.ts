import { and, eq, gte, lt, desc, sql, or, ilike } from "drizzle-orm";
import { db } from "@/db/client";
import { transactions, categories, subcategories } from "@/db/schema";
import { monthRange, yearRange } from "@/lib/dates";

/** Net spend (refunds net out), gross expenses, and count for a month. */
...
/** Per-category net totals for a month, biggest first ("where the money went"). */
...
/** Net total per month (1..12) for a year, with empty months as 0. */
...
/** Per-category net totals for a whole year, biggest first. */
